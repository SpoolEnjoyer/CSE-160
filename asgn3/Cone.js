// Cone.js — updated for asgn3 shaders (MVP matrices, u_whichTexture)
// UV attribute is disabled before drawing since cones use solid color.
...
class Cone {
  ...
  static initBuffer(segments) {
    ...
    for (let i = 0; i < segments; i++) {
      ...
      const tip=[0,0.5,0], p1=[x1,-0.5,z1], p2=[x2,-0.5,z2];

      verts.push(tip[0],tip[1],tip[2], p1[0],p1[1],p1[2], p2[0],p2[1],p2[2]);
      const n = computeNormal(tip,p1,p2);
      for (let j=0;j<3;j++) norms.push(n[0],n[1],n[2]);

      verts.push(0,-0.5,0, p2[0],p2[1],p2[2], p1[0],p1[1],p1[2]);
  ...
  render() { Cone.draw(this.matrix, this.color, this.segments); }

  static draw(M, color, segments) {
    Cone.initBuffer(segments);

    // Cone is always solid color — disable UV attribute array so WebGL
    // doesn't try to read past the cube UV buffer's bounds.
    gl.disableVertexAttribArray(a_UV);

    gl.uniform1i(u_whichTexture, -1);   // force color mode
...
function computeNormal(p1,p2,p3) {
  ...
  return len < 1e-5 ? [0,1,0] : [N[0]/len, N[1]/len, N[2]/len];
}
